...
pub mod BotContract{
    ...
    impl BotContract of IBotContract<ContractState> {
        ...
        fn kill_bot(ref self: ContractState) {
            assert(self.executor.read() == get_caller_address() || self.game_contract.read() == get_caller_address() ,'Only admin can kill');
        ...
        fn compute_point(self: @ContractState, seed: u128) -> felt252 {
            ...
            let block_id: u256 = self.get_blockid_from_coordinates(array![(x % self.grid_width.read()),(y % self.grid_height.read())].span()).try_into().unwrap();
